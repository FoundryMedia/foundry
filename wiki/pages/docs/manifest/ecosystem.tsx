...
export default function EcosystemPage(): React.ReactElement {
  ...
  → prefix:     "aap"                  (first letter of each word)`}</code></pre>
        ...
        <ul>
          <li><strong>Primary repo</strong>: exact match on repository name (<code>an-average-platform</code>)</li>
          <li><strong>Satellite repos</strong>: name starts with <code>{"{prefix}-"}</code> (<code>aap-api-lib</code>, <code>aap-billing-service</code>)</li>
        </ul>
