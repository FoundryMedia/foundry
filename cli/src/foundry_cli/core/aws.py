...
Examples::
    ...
    zone_id = resolve_hosted_zone_id("thisistheaverage.com", region="us-east-2")
    print(zone_id)  # "Z0086556CCIT89IR5O5Y"
"""
...
def resolve_hosted_zone_id(
    ...
    Equivalent to::
        ...
        aws route53 list-hosted-zones-by-name \\
            --dns-name thisistheaverage.com \\
            --max-items 1

    Args:
        domain_name: The domain name (e.g., ``thisistheaverage.com``).
        region: Optional region override.

    Returns:
        The hosted zone ID (e.g., ``Z0086556CCIT89IR5O5Y``).
    ...
    # Zone ID comes back as "/hostedzone/Z0086556CCIT89IR5O5Y"
    raw_id = zone["Id"]
...
def resolve_ip_whitelist(
    ...
    - **JSON array** (preferred): ``["45.26.169.185/32", "10.0.0.0/8"]``
    - **JSON object** with key: ``{"emp_ip_whitelist": ["45.26.169.185/32"]}``
    ...
    Returns:
        List of CIDR strings (e.g., ``["45.26.169.185/32"]``).
    """
    raw = get_secret_value(secret_id, region=region)
    data = json.loads(raw)
